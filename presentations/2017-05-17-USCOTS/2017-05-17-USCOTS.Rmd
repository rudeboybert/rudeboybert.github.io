...
* With their [free academic license](https://www.datacamp.com/groups/education), you can
    ...
    + Have access to ALL [their courses](https://www.datacamp.com/courses)
...
* Not a good tool for retention, but for introduction
...
## Chapter 7: Regression Highlights
...
alaska_flights <- alaska_flights %>% 
  mutate(summer = month == 6 | month == 7 | month == 8)
ggplot(data = alaska_flights, mapping = aes(x = dep_delay, y = arr_delay, col=summer)) + 
  geom_point() +
  geom_smooth(method = "lm", se = FALSE)
```
